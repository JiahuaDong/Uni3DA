...
class Modelnet40_data(data.Dataset):
	def __init__(self, pc_root, class_list, status='train', pc_input_num=1024, aug=True, dtype=None):
		...
		if status == 'train':
			if dtype == 'source':
				plist = './dataset/PointDA_data/modelnet_train_s.txt'
			else:
				plist = './dataset/PointDA_data/modelnet_train_t.txt'
			...
			if dtype == 'source':
				plist = './dataset/PointDA_data/modelnet_test_s.txt'
			else:
				plist = './dataset/PointDA_data/modelnet_test_t.txt'
			with open(plist, 'r') as f:
				data = [[line.split()[0], line.split()[1] if len(line.split()) > 1 else '0'] for line in f.readlines()
						if line.strip()]  # avoid empty lines
				self.datas = [x[0] for x in data]
...
class Shapenet_data(data.Dataset):
	def __init__(self, pc_root, class_list, status='train', pc_input_num=1024, aug=True, dtype=None, data_type='*.npy'):
		...
		if status == 'train':
			if dtype == 'source':
				plist = './dataset/PointDA_data/shapenet_train_s.txt'
			else:
				plist = './dataset/PointDA_data/shapenet_train_t.txt'
			with open(plist, 'r') as f:
				data = [[line.split()[0], line.split()[1] if len(line.split()) > 1 else '0'] for line in f.readlines()
						if line.strip()]  # avoid empty lines
				self.datas = [x[0] for x in data]
			...
			if dtype == 'source':
				plist = './dataset/PointDA_data/shapenet_test_s.txt'
			else:
				plist = './dataset/PointDA_data/shapenet_test_t.txt'
			with open(plist, 'r') as f:
				data = [[line.split()[0], line.split()[1] if len(line.split()) > 1 else '0'] for line in f.readlines()
						if line.strip()]  # avoid empty lines
				self.datas = [x[0] for x in data]
...
class Scannet_data_h5(data.Dataset):
	...
	def __init__(self, pc_root, class_list, status='train', pc_input_num=1024, aug=True, dtype=None):
		...
		if self.status == 'train':
			pc_root = './dataset/PointDA_data/scannet/train_scan'
			data_pth = os.listdir(pc_root)
		else:
			pc_root = './dataset/PointDA_data/scannet/test_scan'
			data_pth = os.listdir(pc_root)
...
if __name__ == "__main__":
	...
	# data = Shapenet_data(pc_root='./dataset/PointDA_data/shapenet', class_list=target_class, status='train')
	# data = Shapenet_data1(pc_root='./dataset/PointDA_data/shapenet', status='train')
	# data = Modelnet40_data(pc_root='/home/youhaoxuan/data/Modelnet_Shapenet/modelnet40', status='train')
	data = Scannet_data_h5(pc_root='./dataset/PointDA_data/scannet')
	print(len(data))
	point, label = data[0]
